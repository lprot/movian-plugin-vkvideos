...
new page.Route(plugin.id + ":moreFromTheOwner:(.*)", function(page, id) {
    ...
    function loader() {
        page.loading = true;
        var JSON = showtime.JSONDecode(http.request(API + '/video.get', {
            args: {
                uid: id,
                count: 200,
                offset: page.entries,
...
new page.Route(plugin.id + ":play:(.*):(.*)", function(page, url, title) {
    ...
    if (v.match(/player.molodejj.tv[^']+/)) {
        ...
        JSON = showtime.JSONDecode(http.request(url));
...
function scraper(page, query) {
    ...
    function loader() {
        page.loading = true;
        var JSON = showtime.JSONDecode(http.request(API + '/video.search', {
            args: {
                q: query,
